...
namespace WSICS::Standardization::CxCyWeights
{
	...
	/*cv::Ptr<cv::ml::NormalBayesClassifier> CreateNaiveBayesClassifier2(const cv::Mat& c_x, const cv::Mat& c_y, const cv::Mat& density, const cv::Mat& class_data)
	{
		...
		for (size_t row = 0; row < class_data.rows; ++row)
		{
			for (size_t col = 0; col < class_data.cols; ++col)
			{
				...
				switch ((int32_t)class_data.at<float>(row, col))
				{
